Spree::Payment.class_eval do
  ...
  private
    def register_coinbase_charge
      charge_params = {name: order.number, description: order.line_items.map(&:name).to_sentence, local_price: {amount: amount.to_s, currency: currency.to_s}, pricing_type: 'fixed_price', metadata: {number: number}, redirect_url: "#{payment_method.preferred_redirect_host}/coinbase/redirect"}
